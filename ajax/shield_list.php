...
$sql = "SELECT pd.id AS product_id, pd.product_name, pd.product_price, pd.product_size, pd.product_color, pd.product_desc, pd.product_category, 
        ...
        WHERE pi.img_category='main' AND pd.status = 'Active' AND pd.product_type='shield'
...
if($result = mysqli_query($conn, $sql)) {
    ...
    $totalCountQuery = "SELECT COUNT(*) AS total FROM product_data WHERE status = 'Active' AND product_type='shield'";
